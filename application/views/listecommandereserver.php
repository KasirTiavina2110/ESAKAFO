			...
			<div class="container">
					<div class="row">
							<section class="12u 12u$">
								...
								<a href="<?php echo base_url('liste-commande-pret');?>" class="link"><h1>Liste des commandes reserver:</h1></a>
										<table>
											<thead>
												<tr>
														<th>Statut</th>
											...
											<tbody>
												<?php for($i=0; $i<count($commande_data); $i++) { ?>
													<tr>
															<?php if($commande_data[$i][2]=='3'){;?>
																<form action="<?php echo base_url('commande_controller/updatecommanderecuperer') ;?>" method="post">
																	...
																	<input name="idlivreur" value="<?php echo $_SESSION['idlivreur'] ;?>" hidden/>
																		<td><input type="submit" value="RECUPERER" class="button alt"/></td>
																</form>	
															<?php }else{;?>
																	<?php for($a=2; $a<count($commande_data[$i]); $a++) { ?>
																		<td><input name="<?php echo 'a'.$a;?>" type="text" value="<?php echo $commande_data[$i][$a] ;?>"/></td>
